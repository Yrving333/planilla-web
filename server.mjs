...
// --------- ENV ----------
const PORT         = Number(process.env.PORT) || 10000;         // Render inyecta PORT
...
// --------- ESTÁTICOS (soluciona "Cannot GET /") ----------
...
app.get("/api/login", async (req, res) => {
  try {
    ...
    // Usa vista si existe; si falla, consulta directa
    let row;
    try {
      const r = await q(
...
app.post("/api/planillas", async (req, res) => {
  ...
  if (total > TOPE + 1e-6) {
    return res.status(400).json({ msg: `Has superado el tope de S/ ${TOPE.toFixed(2)}` });
  }
  ...
  try {
    ...
    res.json({ ok: true, serie, num, total, fecha, empresa: emp || null });
  } catch (e) {
    console.error(e);
